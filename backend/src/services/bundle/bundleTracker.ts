 ...
 *
 * Detection gate:
 *   isJitoBundle=true only — a non-dev wallet spent ≥7 SOL in the SAME BLOCK as
 *   the token's creation, confirmed by pumpPortalAdapter's RPC retrocheck
 *   (bonding-curve balance-diff parsing).
 *
 * Fast-follow buys (T+1–15s via WebSocket) are explicitly excluded: backtesting
 * shows they have a 23% win rate and -8.6% avg PnL vs Jito's 68.8% / +109%.
 *
 * Scoring 0–100:
 *   60  — base: first ≥7 SOL Jito buy
...
export class BundleTracker {
  ...
  /** No-op — Jito-only detection needs no launch-time bookkeeping. */
  ...
  onTrade(event: CanonicalEvent): void {
    ...
    // Only act on same-block Jito buys confirmed by the RPC retrocheck.
    // Fast-follow (T+1-15s) buys are excluded: 23% win rate, -8.6% avg PnL.
